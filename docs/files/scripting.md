...
GAME.DEFAULT.disable(); // disables default player logic
GAME.DEFAULT.disable3D(); // disable 3D Scene (For 2D Only Games)
GAME.DEFAULT.disableServer(); // disables all multiplayer/networking
GAME.UI.disableDefault(); // disables most default krunker UI
```
...
action drive(num delta) {
    if (car.fuel > 0) { // car only moves while it has fuel
        car.x += car.speed * delta; // move car
